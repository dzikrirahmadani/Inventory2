$(document).ready(function(){
        ...
        $.ajax({
            ...
            success : function(response){
                ...
                for( let i = 0; i < response.length; i++){
                    barang_merk += showTabel(response, i);
                }
                document.getElementById('targetData').innerHTML = barang_merk;
                ...
                btn_ubah.forEach(el => {
                    el.addEventListener('click', function() {
                        ...
                        document.location.href = `ubah-merk.html?data=${id}`;
                    })
                })
    ...
    function showTabel(response, i){
        return `
            <tr>
                <td>${response[i].nm_merk}</td>
                <td class="aksi flex">
                    <button class='btn-edit px-2 p-1 bg-green-600 text-white rounded-[3px]' data-id="${response[i].id_merk}" id="btn-ubah">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                        <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z"/>
                        <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z"/>
                    </svg>
                    </button>
                    <button class='ml-1 btn-hapus px-2 p-1 bg-orange-600 text-white rounded-[3px]' data-id="${response[i].id_merk}" id="btn-hapus">
